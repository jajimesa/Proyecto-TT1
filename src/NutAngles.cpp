...
//------------------------------------------------------------------------------
/**
 * @brief Calcula los ángulos de nutación.
 * @param Mjd_TT Fecha juliana modificada (Tiempo Terrestre).
 * @param dpsi [out] Corrección en longitud.
 * @param deps [out] Corrección en oblicuidad.
 */
//------------------------------------------------------------------------------
void NutAngles(double Mjd_TT, double& dpsi, double& deps)
{
    ...
    double l  = mod(  485866.733 + (1325.0*rev +  715922.633)*T
                                   + 31.310*T2 + 0.064*T3, rev );
    double lp = mod( 1287099.804 + (  99.0*rev + 1292581.224)*T
                                      -  0.577*T2 - 0.012*T3, rev );
    double F  = mod(  335778.877 + (1342.0*rev +  295263.137)*T
                                      - 13.257*T2 + 0.011*T3, rev );
    double D  = mod( 1072261.307 + (1236.0*rev + 1105601.328)*T
                                        -  6.891*T2 + 0.019*T3, rev );
    double Om = mod(  450160.280 - (   5.0*rev +  482890.539)*T
